...
export default function Comment() {
    ...
    return <>
        <div style={{display: 'grid', width: '100%', gridTemplateColumns: '3em 1fr 5em'}}>
            ...
            <button onClick={()=>{
                const cmt :HTMLTextAreaElement|null = document.querySelector('textarea')
                if(cmt&&cmt.value==='') return
                else {
                    setcomment([{name:'You',comment: cmt? cmt.value : '' },...comment_list])
                    if(cmt) cmt.value=''
                }
            } }>Send</button>
        </div>
        <div>
            {comment_list.map((el:{comment: string,name: string},index: number)=><div key={index} style={{display: 'grid', width: '100%', gridTemplateColumns: '3em 1fr'}}>
                <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="currentColor"
                     className="bi bi-file-person-fill" viewBox="0 0 16 16">
                    <path
                        d="M12 0H4a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2zm-1 7a3 3 0 1 1-6 0 3 3 0 0 1 6 0zm-3 4c2.623 0 4.146.826 5 1.755V14a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1v-1.245C3.854 11.825 5.377 11 8 11z"/>
                ...
                <div >
                    <div>
                        <b style={{color:'#0285b5'}}>{el.name}</b>
                        <p>{el.comment}</p>
                    </div>
